<?php
include 'C:/xampp/htdocs/CosmicWonders/BackEnd/config.php'; // Đường dẫn đến file config
...
if ($result) {
    if ($result->rowCount() > 0) {
        ...
        echo '</div>'; // Đóng div.category
        echo '</div>'; // Đóng div.ship_categories
        echo '<hr>';
        ...
        foreach ($spaceships as $shipname => $ships) {
            ...
            echo '</div>'; // Đóng div.ship
        }
        echo '</div>'; // Đóng div.ship-list
        echo '</div>'; // Đóng div.space-list
    } else {
