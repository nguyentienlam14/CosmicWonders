<?php
// SQL query để lấy tất cả phi hành gia và chi tiết của họ
...
if (!empty($astronauts)) {
    ...
    foreach ($groupedAstronauts as $astronautId => $detailsByType) {
        if (!empty($detailsByType[1])) {
            echo '
                <div class="astronaut-header p-relative start">
                    <img class="img-fuild" src="' . htmlspecialchars($detailsByType[1][0]['Astronaut_detail_img']) . '" alt="">
                    <div class="overlay"></div>
                    <h2 class="fs-sm-5 fs-1">' . htmlspecialchars($detailsByType[1][0]['Astronaut_title']) . '</h2>
                    <div class="astronaut-header-sub">
                        <span>' . htmlspecialchars($detailsByType[1][0]['Astronaut_subtitle']) . '</span>;
                    </div>
        ...
        if (!empty($detailsByType[2])) {
            echo '
                <div class="astronaut-header p-relative start">
                    <img class="img-fuild" src="' . htmlspecialchars($detailsByType[2][0]['Astronaut_detail_img']) . '" alt="">
                    <div class="overlay"></div>
                    <h2 class="fs-sm-5 fs-1">' . htmlspecialchars($detailsByType[2][0]['Astronaut_title']) . '</h2>
                    <div class="astronaut-header-sub">
                        <span>' . htmlspecialchars($detailsByType[2][0]['Astronaut_subtitle']) . '</span>;
                    </div>
                    <button class="readmore">Read more <i class="bi bi-arrow-down-circle-fill"></i></button>
                </div>';
            echo '<div class="details p-3">';
            echo '<div class="destination row col-lg-9 m-auto">';
            echo '<div class="row mt-lg-5 mb-lg-5 m-0">';
            ...
            echo'</div>';//đóng row
             ...
             echo '</div></div>'; //đóng details và destination
        }
