...
if (isset($_GET['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        foreach ($detailHeaderText as $index => $headerText) {
            ...
            if (isset($_FILES['detailImage']['name'][$index]) && $_FILES['detailImage']['error'][$index] == 0) {
                ...
                $detailTargetDirectory = "C:/Xamp/htdocs/CosmicWonders/Front-end/assets/img/body/";
                ...
                if (!is_dir($detailTargetDirectory) || !is_writable($detailTargetDirectory)) {
                    echo '<script>Swal.fire("Oops!", "Directory for detail images does not exist or is not writable.", "error")</script>';
                    exit();
                }
                ...
                if (move_uploaded_file($_FILES['detailImage']['tmp_name'][$index], $detailTargetFile)) {
                    ...
                    echo '<script>Swal.fire("Oops!", "Failed to upload detail image.", "error")</script>';
                }
            }
    ...
    echo '<script>Swal.fire("Oops!", "Invalid Astronaut ID!", "error")</script>';
}
?>
